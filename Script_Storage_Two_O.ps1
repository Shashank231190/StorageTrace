...
$jobStorageNetwork = {
    param($storageTracesStop, $perfmonStop, $NetworkTarceStop, $LogName, $ProviderName, $Level, $ID)    
...
function get-iSCSIData($LogPath, $ToolLocation) {
    ...
    try {
        ...
        <#
          For jobStorageNetwork param required
          param($storageTracesStop,$perfmonStop,$NetworkTarceStop,$LogName,$ProviderName,$Level,$ID)   
        ...
        $jobSN = Start-job -ScriptBlock $jobStorageNetwork -ArgumentList @($StorageTracesStop, $perfmonStop, $NetworkTarceStop, $EventLogName, $EventProviderName, $EventLevel, $EventID)  -name "StorageNetwork"
            ...
            switch ($command) {
                ...
                2 { 
                    if ((Get-Job -id $jobSN.ID).state -eq "Completed" -and (Get-Job -id $jobX.ID).state -eq "Completed") {
                        ...
                        Write-Host -ForegroundColor yellow "Throwing dummy eventID $($EventID) to stop the job $($jobSN.Name) & $($JobX.Name)."
